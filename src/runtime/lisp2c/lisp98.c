...
extern CL_INIT XIparse_integer1[];
extern CL_INIT XFparse_integer1[];
...
void parse_integer1(CL_FORM *base)
{
	...
	else
	{
		...
		else
		{
			...
			COPY(ARG(8), ARG(10));
			LOAD_SYMBOL(SYMBOL(Slisp, 410), ARG(11));	/* READTABLE */
			Ferror(ARG(9), 3);
		}
		if(CL_CHARP(ARG(5)))
		{
	...
	if(CL_SYMBOLP(ARG(8)) && GET_SYMBOL(ARG(8)) == SYMBOL(Slisp, 459))	/* WHITESPACE */
	{
	}
	else
	{
