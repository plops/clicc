...
static void Z153_union_internal(CL_FORM *base, CL_FORM *display[]);
static void Z154_g1705(CL_FORM *base, int nargs);
...
void Funion(CL_FORM *base, int nargs)
{
	...
	else
	{
		...
		{
			GEN_CLOSURE(array, STACK(base, 5), 4, Z154_g1705, -1);
	...
	Z153_union_internal(STACK(base, 5), display);
...
static void Z153_union_internal(CL_FORM *base, CL_FORM *display[])
{
	if(CL_TRUEP(STACK(base, 0)))
	{
		...
		{
			Fcdr(STACK(base, 0));
			Z153_union_internal(STACK(base, 0), display);
		}
		else
		{
			COPY(STACK(base, 0), STACK(base, 1));
			...
			COPY(STACK(base, 0), STACK(base, 2));
			Fcdr(STACK(base, 2));
			Z153_union_internal(STACK(base, 2), display);
			{
				CL_FORM *lptr;
