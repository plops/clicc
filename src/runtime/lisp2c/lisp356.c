...
static void Z125_add(CL_FORM *base, CL_FORM *display[]);
...
void merge_directories(CL_FORM *base)
{
	...
	LOAD_BOOL(CL_SYMBOLP(ARG(3)) && GET_SYMBOL(ARG(3)) == SYMBOL(Slisp, 257), ARG(3));	/* ABSOLUTE */
	if(CL_TRUEP(ARG(3)))
	{
		goto THEN1;
	}
...
static void Z125_add(CL_FORM *base, CL_FORM *display[])
{
	if(CL_SYMBOLP(ARG(0)) && GET_SYMBOL(ARG(0)) == SYMBOL(Slisp, 258))	/* BACK */
	{
		if(CL_TRUEP(&display[0][3]))
		{
			COPY(GET_CAR(&display[0][3]), ARG(1));
			LOAD_BOOL(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 258), ARG(1));	/* BACK */
			if(CL_TRUEP(ARG(1)))
			{
				goto ELSE1;
			}
