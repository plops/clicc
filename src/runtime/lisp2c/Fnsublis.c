...
static void Z147_nsublis_internal(CL_FORM *base, CL_FORM *display[]);
static void Z148_g1696(CL_FORM *base, int nargs);
...
void Fnsublis(CL_FORM *base, int nargs)
{
	...
	else
	{
		...
		{
			GEN_CLOSURE(array, STACK(base, 5), 4, Z148_g1696, -1);
	...
	Z147_nsublis_internal(STACK(base, 5), display);
...
static void Z147_nsublis_internal(CL_FORM *base, CL_FORM *display[])
{
	...
	else
	{
		...
		Fcar(STACK(base, 2));
		Z147_nsublis_internal(STACK(base, 2), display);
		COPY(STACK(base, 0), STACK(base, 3));
		Fcdr(STACK(base, 3));
		Z147_nsublis_internal(STACK(base, 3), display);
		COPY(STACK(base, 0), STACK(base, 4));
		COPY(STACK(base, 2), STACK(base, 5));
		Frplaca(STACK(base, 4));
		COPY(STACK(base, 0), STACK(base, 4));
		COPY(STACK(base, 3), STACK(base, 5));
		Frplacd(STACK(base, 4));
	}
}
