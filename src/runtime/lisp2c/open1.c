...
extern CL_INIT XIopen1[];
extern CL_INIT XFopen1[];
CL_INIT Kopen1[] =
{
	MAKE_STRING(1, "r"),	/* 0 */
...
static void Z64_get_io_file(CL_FORM *base, CL_FORM *display[]);
...
void open1(CL_FORM *base)
{
	...
	if(caller_base == NULL)
	{
		...
		{
			if(CL_SYMBOLP(ARG(3)) && GET_SYMBOL(ARG(3)) == SYMBOL(Slisp, 218))	/* ERROR */
			{
			...
			else
			{
				if(CL_TRUEP(ARG(3)))
				{
					if(CL_SYMBOLP(ARG(3)) && GET_SYMBOL(ARG(3)) == SYMBOL(Slisp, 224))	/* CREATE */
					{
						COPY(ARG(0), ARG(6));
		...
		LOAD_BOOL(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 216), ARG(7));	/* INPUT */
		if(CL_TRUEP(ARG(7)))
		{
			goto THEN1;
		}
		...
		if(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 225))
		{
			THEN1:;
			if(CL_TRUEP(ARG(6)))
			{
				...
				COPY(ARG(6), ARG(8));
				COPY(ARG(8), ARG(9));
				LOAD_SMALLFIXNUM(9, ARG(10));
				COPY(ARG(9), ARG(11));
				LOAD_SYMBOL(SYMBOL(Slisp, 64), ARG(12));	/* STREAM */
				rt_struct_typep(ARG(11));
				if(CL_TRUEP(ARG(11)))
				{
					COPY(OFFSET(AR_BASE(GET_FORM(ARG(9))), GET_FIXNUM(ARG(10)) + 1), ARG(8));
				}
				else
				{
			...
			if(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 225))	/* PROBE */
			{
				...
				COPY(ARG(7), ARG(9));
				COPY(ARG(9), ARG(10));
				LOAD_SMALLFIXNUM(9, ARG(11));
				COPY(ARG(10), ARG(12));
				LOAD_SYMBOL(SYMBOL(Slisp, 64), ARG(13));	/* STREAM */
				rt_struct_typep(ARG(12));
				if(CL_TRUEP(ARG(12)))
				{
					COPY(OFFSET(AR_BASE(GET_FORM(ARG(10))), GET_FIXNUM(ARG(11)) + 1), ARG(9));
				}
				else
				{
		...
		{
			if(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 220))	/* OUTPUT */
			{
				display[0] = ARG(0);
				Z64_get_io_file(ARG(7), display);
				COPY(ARG(0), ARG(8));
			...
			{
				if(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 221))	/* IO */
				{
					display[0] = ARG(0);
					Z64_get_io_file(ARG(7), display);
					COPY(ARG(0), ARG(8));
...
static void Z64_get_io_file(CL_FORM *base, CL_FORM *display[])
{
	if(CL_TRUEP(&display[0][5]))
	{
		...
		c_fclose(ARG(0));
		if(CL_SYMBOLP(&display[0][2]) && GET_SYMBOL(&display[0][2]) == SYMBOL(Slisp, 218))	/* ERROR */
		{
		...
		{
			if(CL_SYMBOLP(&display[0][2]) && GET_SYMBOL(&display[0][2]) == SYMBOL(Slisp, 222))	/* OVERWRITE */
			{
			...
			{
				if(CL_SYMBOLP(&display[0][2]) && GET_SYMBOL(&display[0][2]) == SYMBOL(Slisp, 223))	/* APPEND */
				{
					COPY(&display[0][0], ARG(0));
				...
				{
					LOAD_BOOL(CL_SYMBOLP(&display[0][2]) && GET_SYMBOL(&display[0][2]) == SYMBOL(Slisp, 227), ARG(0));	/* SUPERSEDE */
					if(CL_TRUEP(ARG(0)))
					{
						goto THEN1;
					}
					...
					if(CL_SYMBOLP(&display[0][2]) && GET_SYMBOL(&display[0][2]) == SYMBOL(Slisp, 228))
					{
						THEN1:;
						COPY(&display[0][0], ARG(0));
