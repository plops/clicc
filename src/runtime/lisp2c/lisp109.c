...
void Ffile_namestring(CL_FORM *base)
{
	...
	else
	{
		...
		else
		{
			...
			{
				LOAD_SMSTR((CL_FORM *)&KFfile_namestring[0], STACK(base, 1));	/* etypecase: the value ~a is not a legal value */
				COPY(STACK(base, 0), STACK(base, 2));
	...
	if(CL_TRUEP(STACK(base, 2)))
	{
		...
		host_unparse_file(STACK(base, 3));
	...
	{
		LOAD_SMSTR((CL_FORM *)&KFfile_namestring[2], STACK(base, 0));	/* Cannot determine the namestring for pathnames with no host:~%  ~S */
