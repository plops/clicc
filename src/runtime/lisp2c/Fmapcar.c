...
static void Z69_mapcar_internal(CL_FORM *base, CL_FORM *display[]);
...
void Fmapcar(CL_FORM *base, int nargs)
{
	...
	Z69_mapcar_internal(STACK(base, 3), display);
	COPY(STACK(base, 3), STACK(base, 0));
}

static void Z70_get_rest_args(CL_FORM *base, CL_FORM *display[]);
...
static void Z69_mapcar_internal(CL_FORM *base, CL_FORM *display[])
{
	...
	if(caller_base == NULL)
	{
		...
		else
		{
			...
			Z70_get_rest_args(STACK(base, 3), display);
			Fapply(STACK(base, 1), 3);
			mv_count = 1;
			Z69_mapcar_internal(STACK(base, 2), display);
			{
				CL_FORM *lptr;
...
static void Z70_get_rest_args(CL_FORM *base, CL_FORM *display[])
{
	...
	else
	{
		...
		Fcdr(STACK(base, 2));
		Z70_get_rest_args(STACK(base, 2), display);
		{
			CL_FORM *lptr;
			lptr = form_alloc(STACK(base, 3), 2);
			COPY(STACK(base, 1), CAR(lptr));
