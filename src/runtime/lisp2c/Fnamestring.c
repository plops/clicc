...
void Fnamestring(CL_FORM *base)
{
	...
	else
	{
		...
		else
		{
			...
			{
				LOAD_SMSTR((CL_FORM *)&KFnamestring[0], STACK(base, 1));	/* etypecase: the value ~a is not a legal value */
				COPY(STACK(base, 0), STACK(base, 2));
	...
	{
		LOAD_SMSTR((CL_FORM *)&KFnamestring[2], STACK(base, 0));	/* Cannot determine the namestring for pathnames with no host:~%  ~S */
