...
void Fslot_value(CL_FORM *base)
{
	...
	if(CL_TRUEP(STACK(base, 3)))
	{
		...
		if(CL_TRUEP(STACK(base, 4)))
		{
			...
			LOAD_SMSTR((CL_FORM *)&KClisp[16], STACK(base, 5));	/* The slot ~s is unbound in the object ~s of class ~s. */
		...
		{
			COPY(OFFSET(AR_BASE(GET_FORM(STACK(base, 0))), 1 + GET_FIXNUM(STACK(base, 3))), STACK(base, 0));
		}
	}
	else
	{
		...
		LOAD_SYMBOL(SYMBOL(Slisp, 130), STACK(base, 6));	/* SLOT-VALUE */
