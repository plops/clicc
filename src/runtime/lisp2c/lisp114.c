...
extern CL_INIT XImake_hash_table1[];
extern CL_INIT XFmake_hash_table1[];
...
void make_hash_table1(CL_FORM *base)
{
	...
	{
		if(CL_SYMBOLP(ARG(0)) && GET_SYMBOL(ARG(0)) == SYMBOL(Slisp, 401))	/* EQ */
		{
		...
		{
			if(CL_SYMBOLP(ARG(0)) && GET_SYMBOL(ARG(0)) == SYMBOL(Slisp, 402))	/* EQL */
			{
			...
			{
				if(CL_SYMBOLP(ARG(0)) && GET_SYMBOL(ARG(0)) == SYMBOL(Slisp, 403))	/* EQUAL */
				{
					LOAD_GLOBFUN(&CFequal, ARG(0));
				}
				else
				{
					if(CL_SYMBOLP(ARG(0)) && GET_SYMBOL(ARG(0)) == SYMBOL(Slisp, 404))	/* EQUALP */
					{
