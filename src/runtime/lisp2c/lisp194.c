...
void Fmake_echo_stream(CL_FORM *base)
{
	...
	{
		GEN_CLOSURE(array, ARG(4), 5, Z95_lambda, 0);
		COPY(ARG(2), &array[3]);
		COPY(ARG(0), &array[4]);
	...
	COPY(ARG(4), ARG(4));
	{
		GEN_CLOSURE(array, ARG(5), 4, Z94_lambda, 1);
		COPY(ARG(2), &array[3]);
		LOAD_CLOSURE(array, ARG(5));
	}
	COPY(ARG(5), ARG(5));
	{
		GEN_CLOSURE(array, ARG(6), 4, Z93_lambda, 1);
		COPY(ARG(1), &array[3]);
		LOAD_CLOSURE(array, ARG(6));
	}
	COPY(ARG(6), ARG(6));
	{
		GEN_CLOSURE(array, ARG(7), 4, Z92_lambda, 0);
		COPY(ARG(1), &array[3]);
...
static void Z95_lambda(CL_FORM *base)
{
	...
	{
		LOAD_SMALLFIXNUM(0, ARG(2));
		if(EQL(INDIRECT(GET_FORM(ARG(0)) + 3), ARG(2)))
		{
			COPY(ARG(1), ARG(0));
		}
		else
