...
extern CL_INIT XIexport1[];
extern CL_INIT XFexport1[];
CL_INIT Kexport1[] =
{
...
void export1(CL_FORM *base)
{
	...
	COPY(&mv_buf[0], ARG(5));
	{
		...
		mv_count = 1;
		{
			...
			if(CL_SYMBOLP(ARG(2)))
			{
				GET_SYM_NAME(ARG(9), ARG(2), ARG(8));
			}
			else
			{
				if(CL_TRUEP(ARG(2)))
				{
					COPY(SYMVAL(Slisp, 679), ARG(8));	/* SYM_EXPECTED */
					COPY(ARG(2), ARG(9));
					Ferror(ARG(8), 2);
				}
				else
				{
					LOAD_SMSTR((CL_FORM *)&KClisp[243], ARG(8));	/* NIL */
				}
			}
			COPY(ARG(6), ARG(9));
			find_symbol1(ARG(8));
					...
					if(CL_TRUEP(ARG(9)))
					{
						...
						else
						{
							...
							else
							{
								...
								COPY(ARG(6), ARG(11));
								LOAD_SYMBOL(SYMBOL(Slisp, 344), ARG(12));	/* PACKAGE */
								Ferror(ARG(10), 3);
							}
							COPY(ARG(8), ARG(11));
							COPY(ARG(10), ARG(12));
