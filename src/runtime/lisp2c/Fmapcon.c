...
static void Z77_mapcon_internal(CL_FORM *base, CL_FORM *display[]);
...
void Fmapcon(CL_FORM *base, int nargs)
{
	...
	Z77_mapcon_internal(STACK(base, 3), display);
	COPY(STACK(base, 3), STACK(base, 0));
}

static void Z78_get_rest_args(CL_FORM *base, CL_FORM *display[]);
...
static void Z77_mapcon_internal(CL_FORM *base, CL_FORM *display[])
{
	...
	if(caller_base == NULL)
	{
		...
		else
		{
			...
			Z78_get_rest_args(STACK(base, 3), display);
			Fapply(STACK(base, 1), 3);
			mv_count = 1;
			Z77_mapcon_internal(STACK(base, 2), display);
			Fnconc(STACK(base, 1), 2);
...
static void Z78_get_rest_args(CL_FORM *base, CL_FORM *display[])
{
	...
	else
	{
		...
		Fcdr(STACK(base, 2));
		Z78_get_rest_args(STACK(base, 2), display);
		{
			CL_FORM *lptr;
