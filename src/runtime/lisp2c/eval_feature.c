...
extern CL_INIT XIeval_feature[];
extern CL_INIT XFeval_feature[];
...
void eval_feature(CL_FORM *base)
{
	if(CL_ATOMP(ARG(0)))
	{
		COPY(SYMVAL(Slisp, 409), ARG(1));	/* *FEATURES* */
		LOAD_NIL(ARG(2));
		LOAD_NIL(ARG(3));
		LOAD_NIL(ARG(4));
		member1(ARG(0));
	...
	else
	{
		...
		if(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 463))	/* NOT */
		{
		...
		{
			if(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 464))	/* AND */
			{
				LOAD_NIL(ARG(2));
			...
			{
				if(CL_SYMBOLP(ARG(1)) && GET_SYMBOL(ARG(1)) == SYMBOL(Slisp, 465))	/* OR */
				{
					LOAD_NIL(ARG(2));
					COPY(GET_CDR(ARG(0)), ARG(3));
					M2_1:;
